...
| `ith-monsoon.js` | The vibrant `monsoon-minds.html` buy page: pre-launch gate (countdown + discreet password `sid@1001`), the **47 real events** from the two brochures (23 Science & Tech @ ₹600/participant, 24 Multidisciplinary @ ₹400/participant; price = rate × participants), a category filter, cart with promo codes (`MONSOON1` forces ₹1 for testing — remove before launch) and a **40% school discount + Rolling-Trophy eligibility at 15+ events**, terms gate, then `POST /api/create-order` → Razorpay → success.html. Brochure PDFs in `assets/brochure/`. |
