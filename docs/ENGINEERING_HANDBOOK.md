...
| `ith-monsoon.js` | The vibrant `monsoon-minds.html` buy page: pre-launch gate (countdown + discreet password `sid@1001` → `sessionStorage.mm_ok`), fictional competition catalogue (editable `COMPS`), cart with promo codes (`MONSOON1` forces total to ₹1 for testing — remove before launch) and 50% school bulk discount, terms gate, then `POST /api/create-order` → Razorpay → success.html. |
